...
def get_account_id_from_name(creds, user_name: str) -> Optional[str]:
...
def run_jira_jql(creds, jql: str, fields: str = "summary,status,created") -> List[dict]:
...
def query_tasks_by_username(creds, username: str):
...
def query_tasks_by_date_and_optional_username(creds, from_date: str, to_date: str, username: Optional[str] = None):
...
def query_tasks_by_status_and_optional_username(creds, status: str, username: Optional[str] = None):
...
def resolve_special_dates(from_date_str: str, to_date_str: str):
...
def parse_jira_query_with_mistral(user_input: str):
    today = datetime.utcnow().date()
    yesterday = today - timedelta(days=1)
    prompt = f'''
...
@app.post("/chat", response_model=JiraTaskListResponse)
